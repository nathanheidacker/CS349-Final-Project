...
def covered_calls(portfolio, security, start_date=None, end_date=None, expiry_schedule=None, strike_adjustment=None):
	"""
	Performs a covered call strategy on a portfolio using the input security, 
	given a start date. If no end date is given, the algorithm will run until
	the present day
	...
	Arguments:
		...
		security {Stock}:
			a finance.Stock object that the algorithm uses as
			the underlying asset for the calls it sells.
	...
	while portfolio.date <= end_date:
		...
		if portfolio.date.weekday() < 5:
			...
			stock_purchase_volume = math.floor(portfolio.liquid().value / security.price / 100) * 100
			if stock_purchase_volume > 0:
				portfolio.buy(security, stock_purchase_volume)

			# Calculate current spy volume and spy call volume
			current_security = portfolio.position(security)
			current_security_volume = getattr(current_security, "volume", 0)
			current_security_call_volume = sum([position.volume for position in portfolio.positions.get("call", []) if position.asset.name == security.name])

			# The call to be sold
			basis = getattr(current_security, "basis", None)
			security_call = finance.Call(security, max(math.floor(security.price + strike_adjustment), math.floor(basis + 1)), expiry_schedule[portfolio.date.weekday()])

			# Decrement stock position basis by premium collected (per share)
			#current_security.basis -= (security_call.price / 100)

			# Determine number of calls that can be sold out
			call_short_volume = (current_security_volume // 100) - current_security_call_volume
			if call_short_volume > 0:
				portfolio.short(security_call, call_short_volume)
